...
TINY.table = function () {
     ...
     * @param f  1
     */
    sorter.prototype.init = function (e, f,tb) {
        var t = ge(e), i = 0;
        this.e = e; // this.e = talbe
        this.l = t.r.length; // 行数
        t.a = [];
        t.h = T$$('thead', T$(tb))[0].rows[0]; // 获取表头
        t.w = t.h.cells.length; // 列数量
        for (i; i < t.w; i++) {
            var c = t.h.cells[i]; // 给表头添加click方法
            if (c.className != 'nosort') {

                c.className = this.head;// 'head' 改变头样式
