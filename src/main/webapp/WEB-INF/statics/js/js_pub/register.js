...
(function($) {
	...
	require.prototype = {
		_init: function(b) {
			...
			var _this = this;
			if (b.code) {
				$("#verifyYz, #verifyYz_pass").click(function() {

					$("#time_box,#time_box_pass").text("60 s后可重发");
					// alert("yzmClick");
                    $("#yzm_info_reg,#yzm_info_pass").show();//removeClass("hide");
					_this._sendVerify();
				})
			}
			$('body').on({
				blur: function(a) {
					...
					_this.formValidator($(this));
					if (b.phone && $(this).attr("id") === "phone") _this._change($(this));
					b.onBlur ? b.onBlur($(this)) : ''
				},
				focus: function(a) {
					b.onFocus ? b.onFocus($(this)) : $(this).parent().find("label.focus")
						.not(".valid").removeClass("hide").siblings(".valid").addClass("hide")
						&& $(this).parent().find(".blank").addClass("hide")
	...
	j._click = function(c) {
		...
		// alert("d.length" + d.length !== r.length);
	...
	require.defaults = {
		formId: 'verifyCheck,#verifyCheck_pass',
		onBlur: null,
		onFocus: null,
		onChange: null,
		successTip: true,
