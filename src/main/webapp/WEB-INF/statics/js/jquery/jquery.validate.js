function Validator(errClass) {
    ...
    var methods = {
        ...
        mobileAndPhone: function (value, element) {
            return getLength(value, element) == 0 || execReg(value, /^((\(\d{3}\))|(\d{3}\-))?13[0-9]\d{8}?$|15[0-9]\d{8}?$|18[0-9]\d{8}?$/) || execReg(value, /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/);
        },
                ...
                case 2 :
                    for (var i = 0; i < errCount; i++) {
                        try {
                            ...
                            $('#' + spanID).css({
                                position: "absolute", left: $(element).offset().left + $(element).outerWidth(),
